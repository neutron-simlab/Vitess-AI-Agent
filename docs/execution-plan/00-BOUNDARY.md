...
| `agents/backends.py` | `agents/backends.py` | `JUENA_MEMORY_SYSTEM_PROMPT` → `MEMORY_SYSTEM_PROMPT`, and becomes an argument to `build_supervisor_backend` with the current text as its default. It is not JüNA-specific prose — it is a prompt-injection guard |
...
`SandboxExecutionEvidence` becomes **`ExecutionEvidence`** in core, with its fields
unchanged (`attempted`, `succeeded`, `status`, `exit_code`). Nothing in it is
Podman-shaped.
