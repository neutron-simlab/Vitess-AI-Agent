...
class GuideFileListManager(QMainWindow):
    def __init__(self):
        super().__init__()
    ...
    def setup_ui(self):
        ...
        # Title
        title_label = QLabel("Guide Module -- Input Files Manager")
        title_font = QFont()
    ...
    def create_file_list_panel(self) -> QWidget:
        ...
        # File list group
        file_group = QGroupBox("Input File (ShapeFileName)")
    ...
    def add_files(self):
        """Add files to the list (minimum 1, maximum 1)"""
        ...
        if file_paths:
            ...
            if total_files > 1:  # NF_MAX
                excess = total_files - 1
                QMessageBox.warning(
                    self, 
                    "Too Many Files", 
                    f"Cannot add all files. Maximum is 1 file total.\n"
    ...
    def add_file_to_list(self, file_path: str):
        ...
        if len(self.file_paths) >= 1:  # NF_MAX
            QMessageBox.warning(self, "Maximum Files", "Cannot add more files. Maximum is 1 file.")
            return
